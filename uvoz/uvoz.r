...
names(potrosnja_delez)[3] <- "Delez drzavne potrosnje v turizmu (%)"
...
tabela$`Delez drzavne potrosnje v turizmu (%)` <- tabela$`Delez drzavne potrosnje v turizmu (%)` %>% gsub("^([0-9. ]+).*", "\\1", .) %>% gsub(" ","", .) %>% as.numeric()
tabela$`Vrednost drzavne potrosnje v turizmu (bilion $)` <- tabela$`Vrednost drzavne potrosnje v turizmu (bilion $)` %>% gsub("^([0-9. ]+).*", "\\1", .) %>% gsub(" ","", .) %>% as.numeric()
...
najvec_potrosnje <- filter(tabela, tabela$`Vrednost drzavne potrosnje v turizmu (bilion $)` > 1.690)
najvec_investicij <- filter(tabela, tabela$`Vrednost investicij v turizem (bilion $)` > 21.935)
najvec_potrosnje_procent <- filter(tabela, tabela$`Delez drzavne potrosnje v turizmu (%)` > 14)
...
>>>>>>> bd8ae4b49d2caa61164121981045cb21677bfa50
